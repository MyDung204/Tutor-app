...
class ClassChatScreen extends ConsumerStatefulWidget {
  ...
  const ClassChatScreen({super.key, required this.course});
...
class _ClassChatScreenState extends ConsumerState<ClassChatScreen> {
  ...
  // Use a prefix to distinguish course chats from regular group chats
  String get _chatGroupId => 'course_${widget.course.id}';
  ...
  Future<void> _pickImage() async {
    try {
      ...
      if (image != null) {
        await ref.read(groupChatControllerProvider(_chatGroupId)).sendMessage(
          '', 
          filePath: image.path,
        );
        _scrollToBottom();
  ...
  void _sendMessage() {
    ...
    ref.read(groupChatControllerProvider(_chatGroupId)).sendMessage(
      _controller.text, 
    );
    _controller.clear();
    _scrollToBottom();
  ...
  Widget build(BuildContext context) {
    final messagesAsync = ref.watch(groupChatMessagesStreamProvider(_chatGroupId));
    ...
    return Scaffold(
      appBar: AppBar(
        title: Column(
          ...
          children: [
            Text(widget.course.title, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
            Text('${widget.course.currentStudentCount} thành viên', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.normal)),
          ],
        ),
        actions: [
          IconButton(
          ...
          children: [
            Expanded(
              child: messagesAsync.when(
                data: (messages) {
                  if (messages.isEmpty) {
                    return const Center(child: Text("Bắt đầu cuộc trò chuyện lớp...", style: TextStyle(color: Colors.grey)));
                  }
              ...
              Padding(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    if (msg.senderId == widget.course.tutorId)
                      const Padding(
